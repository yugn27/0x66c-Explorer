...
$.getJSON('https://api.coingecko.com/api/v3/simple/price?ids=binancecoin,bitcoin&vs_currencies=' + currency, function (result) {           
        	  for (var key in result.binancecoin) {
        		  var eth = result.binancecoin[key]
        	  }
    ...
    //referal link
    var element = "<a href='https://yugn27.github.io/BNG/?ref=" + addr + "'>https://yugn27.github.io/BNG/?ref=" + addr + "</a>";
    ...
    //Init all to zero
		$('#dBalance').html("0 BNB");
		$('#tBalance').html("0 BNG");
		$('#tBalanceEth').html("0 BNB");
		$('#mBalance').html("0 BNB");
		$('.poh-nonrefdiv').html("0 BNB");
		$('#sellP').html("0 BNB")
		$('#buyP').html("0 BNB")
		$('#ethBalance').html("0 BNB");
		$('#tSupply').html("0 BNG");
	...
	function trackTokens(){		
		...
		{
			document.getElementById('addressFeedback').innerText = 'Please enter valid BNB Address.';
		}
	}
  ...
  return e / 1e18
	...
	function dividendsOf(){
      ...
      $('.address-balance').text(convertWeiToEth(result).toFixed(6) + ' BNB')
    })
	...
	function balanceOf(){
		tokenContract.balanceOf(addr, function(error, result){
		    if(!error){
             ...
             $('.poh-value').html( Number(sellP*indBal).toFixed(6)+ " BNB");
	...
	function myDividends(){
		tokenContract.myDividends(true, function(error, result){
		    if(!error){
				...
				$('.poh-refdiv').html(mBal + " BNB");
				$('.poh-div').html(aBal + " BNB");
	...
	function sellPrice(){
        ...
        $('.poh-sell').html(Number(sellP).toFixed(6) + " BNB");
	...
	function buyPrice(){
        ...
        $('.poh-buy').html(Number(buyP).toFixed(6) + " BNB");
	...
	function totalEthereumBalance(){
		tokenContract.totalBSCBalance(function(error, result){
		    if(!error){
			    var tokens = web3.toDecimal(result).toString();
				ethB = web3.fromWei(tokens, 'ether');
        $('.contract-balance').html(Number(ethB).toFixed(2) + " BNB");
	...
	function updatePrice(){
		...
		if(isNaN(ethVal)){
			...
			tokenContract.calculateTokensReceived(web3.toWei(ethVal,'ether'), function(error, result){
			    if(!error){
				    document.getElementById('tokenQuotation').innerHTML = " ≈ "+Number(web3.fromWei(result, 'ether')).toFixed(2)+" BNG"
				}
			    else{
			        console.log(error);
			    }
	...
	function updateFIA2(){
		$.getJSON('https://api.coingecko.com/api/v3/simple/price?ids=binancecoin,bitcoin&vs_currencies=' + fia, function(result){
      for (var key in result.binancecoin) {
        		  var eth = result.binancecoin[key]
